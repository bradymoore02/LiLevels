...
FOLDER = f'PlotAnimation/{VIDEO_NAME}2'
...
pixelscale = Plot.scalingy(1.52, data)
pixelscalex = Plot.scaling(10, data)
...
for frame in data.keys():
    ...
    plt.ylim(0,7)  
    #plt.axis('equal')
    plt.plot(np.array(data[frame]['x'])*pixelscalex, -Plot.correct(data[frame]['x'], data[frame]['y'],slope_correction)*pixelscale+6.7, label=f'Frame {frame}', marker='x')
